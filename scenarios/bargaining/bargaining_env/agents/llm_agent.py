...
class LLMAgent(AgentExecutor):
	def __init__(self, model: str, system_prompt: Optional[str], custom_decider: Optional[Callable[[str, Optional[list[str]]], str]] = None):
	...
	async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
		...
		if self._custom_decider is not None:
			...
			if options:
				# Ask the model to pick one option index
				choice = self._choose_from_options(prompt, options)
			else:
				choice = self._freeform(prompt)
	...
	def _choose_from_options(self, prompt: str, options: list[str]) -> str:
...
def main():
	...
	async def _serve():
		async with agent_url_cm as agent_url:
			executor = LLMAgent(model=args.model, system_prompt=system_prompt, custom_decider=custom_decider)
