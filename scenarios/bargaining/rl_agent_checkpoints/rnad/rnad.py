...
#makesure load in local openspiel
repo_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
local_open_spiel = os.path.join(repo_root, "open_spiel")
local_pyspiel_build = os.path.join(repo_root, "build", "python")
for p in (local_pyspiel_build, local_open_spiel):
  if p not in sys.path:
    sys.path.insert(0, p)
from open_spiel.python import policy as policy_lib
import pyspiel

print("pyspiel:", getattr(pyspiel, "__file__", pyspiel))
print("open_spiel.policy:", policy_lib.__file__)
...
def _player_others(player_ids: chex.Array, valid: chex.Array,
  ...
  current_player_tensor = (player_ids == player).astype(
      jnp.int32)  # pytype: disable=attribute-error  # numpy-scalars
...
class RNaDConfig:
  ...
  game_name_only: str = "negotiation"
...
class RNaDSolver(policy_lib.Policy):
  ...
  def __init__(self, config: RNaDConfig, game_params: dict):
    self.config = config
  ...
  def init(self):
    ...
    self._game = pyspiel.load_game(self.config.game_name_only, self.game_params) #to work with chris's changes we need to pass game_params
    ...
    self._loss_and_grad = jax.value_and_grad(self.loss, has_aux=False) #gradients of loss fn, has_aux=False means loss fn returns a single value not a tuple
    ...
    self.optimizer = optax_optimizer( #need to update as self.config.adam is a dataclass not dict
        self.params,
        optax.chain(
  ...
  def loss(self, params: Params, params_target: Params, params_prev: Params,
    ...
    for player in range(self._game.num_players()):
      ...
      v_target_, has_played, policy_target_ = v_trace(
          jnp.expand_dims(v_target[:, :, player], axis=-1),
          ts.env.valid,
          ts.env.player_id,
          ts.actor.policy,
          policy_pprocessed,
    ...
    #                     has_played_list)
    transformed_v = [jnp.expand_dims(v[..., p], axis=-1)
                     for p in range(self._game.num_players())]

    # chex.assert_equal_shape([transformed_v, jnp.array(v_target_list)])
    loss_v = get_loss_v(transformed_v, v_target_list,
                        has_played_list)
  ...
  # def __getstate__(self):
    ...
    # return dict(
        #RNaD config.
        # config=self.config,
        # game_params=self.game_params,               # <-- add
  ...
  def __getstate__(self):
      ...
      return dict(
        # RNaD config.
        config=self.config,
        # Persist game params so we can rebuild the OpenSpiel game on load.
        game_params=getattr(self, "game_params", {
  ...
  # Restore game params BEFORE init; fallback for older checkpoints.
    self.game_params = state.get("game_params", {
    "enable_proposals": True,
    "enable_utterances": False,
    "num_items": 3,
    "discount": 0.9,
